...
const rawRows = [
  ...
  { orderId: "T-3", "Creada en": "03/09/2026 11:00", "Completada en": "03/09/2026 11:30", "Estatus de orden": "CANCELLED", "Repartido por": "RAPPI_CARGO", "Cooking Time": 5, "Tiempo total de envio sin cooking time": 25, Restaurant: "Lucky", Zona: "Centro", Ciudad: "CDMX" },
];
...
const result = await exportDeliveryAnalysis({ rows: dataset.analyzed, stageRows: dataset.stageUniverse, dataset, mapping, filters: { start: "", end: "", day: "", hour: "", restaurant: "", zone: "", city: "", provider: "", brand: "" }, sourceFile: "prueba.xlsx" });
...
const stages = workbook.getWorksheet("Tiempos por etapa");
assert.equal(stages.getCell("A5").value, "Total universo de etapas");
assert.equal(stages.getCell("B5").value, dataset.stageUniverse.length);
assert.equal(stages.getCell("B5").value, 3);
